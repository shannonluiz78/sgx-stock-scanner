...
# Ticker list for SGX stocks (Yahoo Finance uses '.SI' extension)
SGX_TICKERS = [
    "D05.SI",  # DBS Group
    "O39.SI",  # OCBC
    "U11.SI",  # UOB
...
BATCH_SIZE = 10     # Process stocks in chunks of 10
DELAY_SECONDS = 2   # 2-second pause between batches to prevent rate limiting

def fetch_stock_data():
    results = []
    total_tickers = len(SGX_TICKERS)
    print(f"Starting analysis for {total_tickers} SGX stocks...")
    ...
    for i in range(0, total_tickers, BATCH_SIZE):
        ...
        for ticker_symbol in batch:
            try:
                ...
                if hist.empty or len(hist) < 2:
                    print(f"⚠️ Warning: Insufficient price history for {ticker_symbol}, skipping.")
                    continue
        ...
        # Respectful delay between batches to avoid IP blocks
...
def main():
    ...
    print(f"\n🎉 Successfully updated data.json with {len(stock_data)} stocks!")
